...
class _RoomInstructionsScreenState extends State<RoomInstructionsScreen> {
  ...
  void initState() {
    ...
    if (widget.showIndoorNavigation) {
      // Show a welcome message when arriving at the room
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('You have arrived at ${widget.room.name}'),
        ...
        if (image != null) {
          ...
          try {
            final response = await cloudinary.uploadFile(
              CloudinaryFile.fromFile(file.path, folder: 'room_instructions'),
            );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          try {
            // Get the building object first
            final building = await _buildingService.getBuilding(
              widget.room.buildingId,
            );
            ...
            if (building == null) {
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text(
                      'Could not find the building. Please try again.',
                    ),
                    backgroundColor: Colors.red,
                  ),
                );
            ...
            if (building.latitude == null || building.longitude == null) {
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Building location is not available.'),
                    backgroundColor: Colors.red,
                  ),
                );
              }
            ...
            if (mounted) {
              // Navigate to map screen with the building selected
                ...
                MaterialPageRoute(
                  builder:
                      (context) => MainScreen(
                        ...
                        targetRoomId: widget.room.roomId,
                        targetFloorId: widget.room.floorId,
