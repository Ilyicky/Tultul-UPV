    ...
    try {
      ...
      if (image != null) {
        ...
        try {
          ...
          await _buildingService.updateBuildingImage(building.buildingId, response.secureUrl);
                  ...
                  children: [
                    Expanded(
                      child: TextFormField(
                        ...
                        onSaved: (value) => latitude = double.tryParse(value ?? ''),
                      ),
                    ),
                    ...
                    Expanded(
                      child: TextFormField(
                        ...
                        keyboardType: TextInputType.number,
                        onSaved: (value) => longitude = double.tryParse(value ?? ''),
                      ),
                    ),
                  ],
                ),
            ...
            onPressed: () async {
              if (formKey.currentState?.validate() ?? false) {
                ...
                final buildingId = await _buildingService.createBuilding({
                  'name': name,
                  'popular_names': popularNames.isEmpty 
                      ? [] 
                      : popularNames.split(',').map((e) => e.trim()).toList(),
                  'college': college,
                  'address': address,
